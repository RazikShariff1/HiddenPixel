...
class SteganographyApp:
    def __init__(self, master):
        ...
        self.process_button = tk.Button(master, text="Hide Data", command=self.process_data, bg="#27ae60", fg="white", font=("Arial", 10))
        ...
        self.process_button.grid(row=1, column=3, padx=10, pady=5)
        self.decrypt_button.grid(row=2, column=3, padx=10, pady=5)
        self.status_label.grid(row=3, column=0, columnspan=4, pady=5)

        # Add interactive mode
        tk.Radiobutton(master, text="Hide Data", variable=self.mode_var, value="Hide", bg="#ececec", font=("Arial", 10)).grid(row=2, column=0, padx=10, pady=5)
        tk.Radiobutton(master, text="Decrypt Data", variable=self.mode_var, value="Decrypt", bg="#ececec", font=("Arial", 10)).grid(row=2, column=1, padx=10, pady=5)
    ...
    def process_data(self):
        ...
        if data and hasattr(self, 'image_path'):
            save_path = filedialog.asksaveasfilename(initialdir=os.path.join(os.path.expanduser('~'), 'Desktop'), defaultextension=".png", filetypes=[("PNG files", "*.png")])
            if save_path:
                try:
                    if self.mode_var.get() == "Hide":
                        ...
                        decrypted_data = decode_data(self.image_path)
